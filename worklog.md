...
---
Task ID: P1 (addendum 2 — deploy pipeline, tutor chain, teacher gate, KG perf)
Agent: main
Task: Operator directive — fix Vercel deployment autonomously, restore tutor generation, continue teacher/papers surfaces, keep student loop improving.

Work Log:
- VERCEL ROOT CAUSE (was REPORTED, now FIXED+VERIFIED): syllabai-web production deployments of commits authored outside the repo-owner's verified GitHub email were BLOCKED by Vercel commit-author verification (seatBlock COMMIT_AUTHOR_REQUIRED, readyStateReason "couldn't find a Git account for the commit author"); last READY deploy was 6005703. Repo-side fix: all future commits authored as SyllabAI <hussaina269a@gmail.com> (docs/DEPLOYMENT.md records the contract); 3f4cd5f went READY and the live bundle grep-confirms the 4a2ffe0 handoff fix ("answered this question correctly") + nodeName client handling.
- TUTOR CHAIN — three independent production defects, all fixed and verified live:
  1. Spring AI 2.0.x asserts PROMPT-level options are the CONCRETE provider class (OpenAiChatModel: Assert.isInstanceOf "Prompt options must be OpenAiChatOptions type"; GoogleGenAiChatModel: direct checkcast). The adapter built a generic ChatOptions whenever a request carried temperature/maxTokens — which the tutor ALWAYS does — so every provider call died as ClassCastException before reaching the network. THE tutor outage root cause (never worked on Render). Fix: per-provider runtime-options factories in LlmChainConfig (openAiRuntimeOptions/genAiRuntimeOptions, GenAI maps maxTokens→maxOutputTokens), injected into SpringAiChatModelAdapter; RuntimeOptionsFactoryTest.
  2. Free-tier catalog rotated in the same window: Gemini 2.5-flash 404s for new accounts (→ gemini-3.6-flash), all classic OpenRouter :free slugs retired (→ nvidia/nemotron-3-super-120b-a12b:free, probe-verified 200), Groq llama-3.3-70b-versatile retired (→ openai/gpt-oss-120b, catalog-probed via a throwaway US-region function through the provided Vercel token; sandbox egress IP is geo-blocked by Groq — the earlier "key dead" verdict was WRONG, corrected). Groq base-url also fixed (was missing /v1 → 404 on every call) and all model names env-bridged (SYLLABAI_GROQ_MODEL etc.) so catalog churn is an env change, not a deploy.
  3. Diagnosability: provider failures now carry real cause class+message into the aggregate 503 ("[groq: ... | gemini: ... | openrouter: ...]") + WARN per provider — this observability is what exposed defects 1 and the base-url bug within one deploy cycle.
  - Embeddings were silently OFF in prod: Render defines SYLLABAI_EMBEDDING_API_KEY, yml bridged only SYLLABAI_EMBEDDING_GEMINI_API_KEY; both names now bridge + text-embedding-004 → gemini-embedding-001 (768 dims, V11 column). Verified registered in Render logs.
  - LIVE VERDICT: browser tutor 200 "openai/gpt-oss-120b · groq · 15.0s · 5 evidence items" with verbatim spec citations (4CH1-1.30/1.17/1.27/1.33/1.26); out-of-domain question honestly refuses citing insufficient evidence (designed fail-safe).
- KG PERF (found in browser E2E): GET /learners/me/knowledge-graph for 4CH1 (370 nodes) took 75-105s and intermittently 502ed through Render's proxy — dashboard/mastery map/recommendations unusable for the REAL subject. Cause: per-node N+1 recursion (~800 round-trips). Fix: buildTree() = 3 bulk queries (findSubtree CTE, new findPartOfEdgesWithin, findMisconceptionFamilyEdgesWithin) + in-memory assembly, semantics preserved (order, V6/V15 fold rules, dedupe). VERIFIED LIVE: 75-105s → 5.5s, same 248KB payload. V6/V15 fold contract tests restubbed onto the bulk surface.
- TEACHER CONTENT GATE (§7 workflow, previously API-only): core additive read endpoint GET /api/v1/teacher/content/exam-papers/{id}/review (ContentReviewService.paperReview — full answer key incl. correct options + misconception feeds + mark points; behind /api/v1/teacher/**; ContentReviewServiceTest) + web TeacherContentView ("Content gate" tab): review queue, expandable per-version review with answer key, validate/reject version + scheme, paper validation disabled until all versions VALIDATED (409 rule made visible), backend messages surfaced verbatim.
- LEARNER PAPERS BROWSER: web PapersView tab over /api/v1/exam-papers (subject-scoped) with validation-state badges, metadata-only (serving boundary untouched), honest empty state for 4CH1 ("content is going through that gate now"). Prod has 0 papers — ingestion to prod DB is the remaining link.
- BROWSER E2E (prod, Chrome): register → dashboard honest-empty for 4CH1 ("0 of 222 topics"); subject switch to CHM; practice MCQ attempt 201 → "Not correct — 0/1 marks, correct answer C, mastery updated"; My state renders human titles ("Mole calculations and reacting masses"); History shows full attempt evidence; Papers empty state; tutor grounded answer (above). Minor observation (not fixed, cosmetic): a transient toast can cover the tab bar right after a tutor answer; self-dismisses on reload.
- Tokens handled per fresh-shell discipline, never persisted (probe helper scripts generated per-command and deleted; Groq key passed only as one-shot header).
- Base commits (§8): core 4343b5a → a5604a9 → 8b55954 → 8a01f73 → 18de295 → 5d4482c → 4d2f2c5 → 2df794e → 4b64aba; web c81c7a8 → 46d803c → 3f4cd5f → e7e35b5; workbench c3029ff → this addendum.

Stage Summary:
- LIVE AND VERIFIED: Vercel pipeline unblocked (commit-author contract), tutor chain end-to-end (groq primary + gemini/or failover + embeddings back on), 4CH1 KG 75-105s → 5.5s, teacher content-validation UI over the full §7 API, learner papers browser, complete student loop in browser.
- EXTERNALLY BLOCKED (none hard): Gemini/OpenRouter remain fallback-capable; provider keys are the operator's to rotate if quota/geo issues recur.
- NEXT highest-value: (1) ingest the T-C04 r2 4CH1 past-paper corpus into the PROD Neon DB (parser drafts → /teacher/content/past-papers or workbench importer) so the new teacher gate has real content to validate and 4CH1 unlocks for students; (2) diagnose the two workbench Vercel projects stuck in ERROR (workbench host question); (3) first-teacher onboarding decision (operator).

---
Task ID: P1-4CH1 (activation session — build + rehearsal + reset-recovery)
Agent: main
Task: ACTIVATE 4CH1 FOR REAL STUDENTS — corpus → prod pipeline, teacher validation production-usable, learner past papers live. Autonomous product-build directive; only stop on genuine irreversible/product/legal/security decisions.

Work Log:
- RECON: prod core UP (Render, warm), web serving the verified e7e35b5-era chunk set; prod bootstrap endpoint absent (expected, pre-deploy); prod history endpoint discovered = GET /api/v1/learners/me/attempts ({learnerId,total,returned,attempts[]}). Prod has zero teacher/admin accounts (demo seeder is local-profile-only; migrations seed no users) → corpus ingestion (teacher-gated /api/v1/teacher/content/glm-ocr/pairs) impossible until a first admin exists.
- FIRST-ADMIN BOOTSTRAP BUILT (core @ 4b64aba): V19__bootstrap_admin_state.sql (single-row PENDING→CONSUMED/EXPIRED state machine, claimed_by FK, no secrets stored) + BootstrapStateStore (FOR UPDATE row-lock SQL access) + BootstrapAdminService (claim legal only while PENDING AND zero ADMIN users; consumes terminally in the claim transaction; @Scheduled terminal EXPIRE after 15-min unused window; syllabai.bootstrap.enabled=false kill switch; 12+ char letters+digits password rule; AUDIT warn log) + BootstrapAdminController (GET /api/v1/auth/bootstrap-status, POST /api/v1/auth/bootstrap-admin; SecurityConfig permitAll for exactly these two paths). Claimant = ADMIN+TEACHER (same shape as the DemoUserSeeder dev admin).
- SERVING-BOUNDARY GAP FIXED (real leak, found during recon): /api/v1/attempts and /api/v1/attempts/structured loaded questions by id WITHOUT the servable spec — paper detail exposes questionIds to any authenticated user, so students could submit attempts against UNVALIDATED imported questions and receive part labels/marks. Fix: both submission paths now enforce ServableQuestionSpec (STRUCTURED requires VALIDATED current version) with fail-closed 404 and zero content echo. Read endpoints were already gated; the write path now matches.
- PERF FIX: ExamPaperController list/get did full-table scans + in-memory filters → derived queries findAllBySubjectIdOrderByCreatedAtDesc / findAllByExamPaperIdOrderByDifficultyAsc (entityGraph options preserved).
- TESTS: 355 unit tests 0 failures (12 new bootstrap tests incl. claim-once/race/expiry/disabled + 2 new boundary tests). Flyway V19 applied live on the campaign DB; claim → CONSUMED → second-claim-409 verified live; bootstrap-status honest through the whole cycle.
- REHEARSAL RIG: scripts/p1-rehearsal/rehearse.sh boots fresh core (prod-like, no seeder) on the campaign DB and dogfoods the ENTIRE activation chain: bootstrap → RBAC probes (student 403 teacher routes, anon 401) → 2-pair ingestion (counts vs campaign state: 2011jun 11q/92parts/14pts, 2012jan 85 parts; embeddingSkipped=true; re-run dedupes as DUPLICATE) → review queue/paper-review answer-key → 409 paper-validate rule → attempt-path boundary → full teacher validation of 2011jun (versions+schemes) → paper VALIDATED → student structured attempt 201 PENDING → history reflects → queue shrinks. First run: 7 PASS/27 FAIL — root cause NOT the code: a stale local-profile core JVM from 07:01 had seeded demo users (incl. ADMIN) into the campaign DB, so the zero-admins gate correctly refused. Identity tables cleaned; script hardened (abcdefgh weak-pw fixture so bean validation doesn't mask the service check; V19 via to_regclass not boot-log; 404 probes carry a shaped part answer; history = /learners/me/attempts). Full re-run = first post-recovery step.
- SANDBOX RESET #2 (mid-task, second time): /home/z/my-project wiped AGAIN; uncommitted core changes + campaign DB + toolchain lost. /tmp/my-project live-mirror (synced 09:54, minutes before) preserved worklog/scripts/corpus/JDK but NOT repos/ (excluded) — private core/web source is PAT-gated and the PAT is gone with the sandbox.
- RECOVERY (all done): workbench tree restored from mirror + re-attached to git history via anonymous clone of the PUBLIC workbench (remote main = 1b83568, a .vercelignore deploy fix that postdates the earlier summary); JDK restored from mirror; Maven 3.9.9 from repo.maven.apache.org (archive.apache.org stalls — known); PG17.11 debs re-downloaded + extracted (incl. server-dev); cluster initdb'd + running (script paths /home/z/toolchain/pgdata, trust auth, syllabai superuser, db syllabai); pgvector 0.8.0 built with `make with_llvm=no` (the clean knob — no clang shim needed) and CREATE EXTENSION verified.
- DURABILITY STAGING (this time the work survives resets): full core change set re-created as ready-to-apply artifacts in scripts/p1-core-changes/ (new-files/ = 5 complete files verbatim; EDITS.md = exact anchored edits for SecurityConfig/AssessmentService/repositories/controller/tests with the first-run lessons folded in); rehearse.sh patched copy in p1-core-changes/apply/; corpus bundles (82, 15MB) copied to data/corpus-bundles (reset-proof); campaign-state-r2.json intact (81 pairs INGESTED locally pre-reset; 2016jan quarantined).
- PROD ACTIVATION DRIVER BUILT: scripts/p1_prod_drive.py — waits for the deploy via bootstrap-status, one-time claim (env credentials only, never persisted), student probe + RBAC check, ingests all 81 non-quarantined pairs through /teacher/content/glm-ocr/pairs with per-pair invariants (INGESTED/DUPLICATE, embeddingSkipped=true), resume journal (download/p1-activation/ingested.json), 5-paper sample verification (student-readable metadata, SUGGESTED state, answer-leak probe), review-queue census, GREEN/ATTENTION verdict. Quarantine rule preserved: 2016jan is NEVER posted (r2 identity defect — QP duplicates Jan-2015).

Stage Summary:
- BUILT+STAGED (verified pre-reset, re-staged now): one-time first-admin bootstrap, attempt-path serving-boundary fix, papers perf queries, full rehearsal rig, production activation driver — everything needed to activate 4CH1 within ~1 hour of unblock.
- VERIFIED LIVE: prod core UP; prod web current chunk set; history endpoint shape; prod has NO admin/teacher (bootstrap is the unblock).
- SINGLE EXTERNAL BLOCKER: operator PAT (GitHub) — required to (a) re-clone PRIVATE syllabai-core/syllabai-web, (b) re-apply+test+push the staged core changes (Render auto-deploys), (c) push the workbench durability commit. Vercel needs no token (git integration + commit-author contract already fixed). No other decision or credential is pending.
- NEXT once PAT lands: clone core → apply staged changes → mvn test → rehearse.sh GREEN → commit+push core (Render deploy) → run p1_prod_drive.py (claim+ingest 81) → teacher-gate browser E2E → learner papers E2E → workbench durability commit+push.

---
Task ID: P1-4CH1 (activation session 2 — PAT unblock → deploy → prod claim/ingest → 3 prod bugs fixed)
Agent: main
Task: Continue on operator PAT: re-clone core, apply staged changes, test, rehearse, push, Render deploy, prod claim + 81-pair ingestion, teacher/learner E2E, workbench durability commit.

Work Log:
- PAT verified (200 on core repo API); private syllabai-core cloned; main had moved to 3dcbfb1 (parallel session's T-031 tooling commits — script-only, no conflicts with staged edits).
- Staged change set applied verbatim (V19 + bootstrap trio + SecurityConfig + attempt-path boundary + papers perf queries + tests). mvn test 355/355 green.
- REHEARSAL BUG #1 (real, production-class): first-admin claim 500 on a fresh DB — users.save() INSERT was still in Hibernate write-behind when the JdbcTemplate consume() UPDATE ran → bootstrap_admin_state.claimed_by FK violation. Unit tests missed it (mocks); the pre-reset rehearsal never exercised claim (stale-ADMIN pollution). Fix: saveAndFlush + test stubs updated. 355/355 green.
- REHEARSAL BUG #2 (real, endpoint-dead): every POST /glm-ocr/pairs died with HttpMessageConversionException "Type definition error: JsonNode" — PairRequestView typed its five payload fields as Jackson 2 com.fasterxml JsonNode while the web layer is Spring Framework 7 / Jackson 3 (tools.jackson). The controller test constructed PairRequestView in Java, bypassing web binding — why the suite stayed green. Fix: tools.jackson JsonNode + JsonMapper in the controller; new regression test deserializes the RAW body through the web mapper. 356/356.
- REHEARSAL FULLY GREEN: 34 PASS / 0 FAIL (bootstrap incl. weak-pw/claim-200/ADMIN+TEACHER/second-claim-409/window-closed; RBAC 401/403/200; ingestion INGESTED with exact counts 11q/92parts/14mp + embeddingSkipped + DUPLICATE dedupe; boundary 404s + 409 rule; full validation of 11 versions + 11 schemes; paper VALIDATED; structured attempt 201 PENDING; history; queue shrink).
- CORE PUSHED (commit-author contract): 2e16e43 bootstrap, fe98ea9 boundary+perf, f29a0a8 Jackson 3 — Render autoDeployed.
- PROD ACTIVATION DRIVER RUN (p1_prod_drive.py): bootstrap-status 401→wait→200 available=true; CLAIMED admin@syllabai.dev as ADMIN+TEACHER (credentials generated 24-char, held in .secrets/p1-admin.env 600, never logged); window closed forever (available=false); student probe registered (teacher routes 403); ingestion: 82 bundles found, 2016jan(1c) quarantined and NEVER posted, 81 attempted → 6 INGESTED, 70 DUPLICATE, 5 errors.
- DUPLICATE DISCOVERY: a PARALLEL session this morning (T-031, commits f2ce501/3dcbfb1 dated 09-14 morning + tag t031-prod-ingestion-20260914) had already ingested the corpus into prod Neon through the same endpoint. Dedup gate correctly refused re-import (70/81) and added 6 papers prod lacked. Prod census: 87 SUGGESTED papers, 810 suggested versions.
- PROD BUG #3 (subject association — the papers were UNREACHABLE): all 87 papers sat in a placeholder Subject code=GEN ("unknown subject", GEN-INGEST DRAFT "ingested, pending review") while /curriculum/subjects serves 4CH1. Subject-scoped paper browsing returned 0 — students could never see any past paper even after validation. Root: the pipeline never guesses curriculum placement (by design, §7) but the review workflow had NO placement step. Local rehearsal's "student sees 2 subject papers" check had passed HOLLOW (subject-id extraction silently failed → unscoped query = all papers).
- FIX (§7 placement): POST /api/v1/teacher/content/exam-papers/{id}/place {subjectId} + placePaper() in ContentReviewService (idempotent, AUDIT-logged, factual association ONLY — validation states/serving boundary untouched); subjectId added to PaperSummary + review PaperHeader; web Placement panel (select + place + honest current-placement text) in TeacherContentView; tsc+eslint clean. Web pushed a69be27.
- PROD BUG #4 (4 pairs 500): uq_question_part violation — parser fragmentation emits duplicate part labels within one version (marks-bearing row + empty continuation). Reproduced locally on a fresh schema with the exact bundles. Fix at the persistence boundary (all ingestion paths): uniquePartLabel() deterministic uniquification (first row keeps printed label → mark refs resolve; later rows get .2/.3) + duplicate-part-label review findings so the teacher merges/rejects (never silently dropped). 361/361 tests; local repro now 201 on all four bundles. Pushed 6fe6f25+69bd302.
- Core deploy of the placement/label-fix line verified in prod by review-header probe (subjectId key pending at last check — Render build queue).
- Ops note: sandbox reaper kills background processes between calls; the driver was re-run in foreground chunks with the resume journal (idempotent, DUPLICATE-safe).

Stage Summary:
- 4CH1 activation is ~90% done: prod has first admin (claim consumed), 87 real past papers (4CH0/4CH1 Edexcel IGCSE Chemistry) SUGGESTED in the teacher queue, 810 versions awaiting teacher validation, placement API+UI shipped, part-label 500s fixed, serving boundary + perf + bootstrap all live.
- REMAINING: (1) wait for the LAST Render deploy (subjectId in review header = marker), then prod E2E via scripts/p1_prod_e2e.py: place 2011jun → 4CH1, validate 11 versions + 11 schemes, validate paper, student sees VALIDATED under 4CH1, structured attempt 201 PENDING, history; (2) re-ingest the 4 fixed pairs (1c-2013junr, 2c-2016jun, 2c-2019junr, 2c-2024junr) and optionally bulk-place the remaining GEN papers into 4CH1 (placement is factual: they ARE Edexcel IGCSE Chemistry; validation stays the human gate); (3) workbench durability commit + push.
